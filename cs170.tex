...
{\scriptsize {\it starts with an empty graph \& selects edges from E repeatedly with lightest weight that does not produce a cycle $\in$ O($\mid$E$\mid$log$\mid$V$\mid$). Uses disjoint sets to determine whether a cycle exists in amortized constant time (see disjoint set data structure).}}
